function About() {
  return (
    <section id="about" className="about">
      <h2>About Me</h2>

      <p>
        I am passionate about crafting stories and building immersive worlds, 
        and bringing them to life through digital media. 
        I specialize in visual storytelling, animation, and video editing, 
        and I also enjoy exploring narrative through writing, particularly in military fiction. 
        My goal is to create engaging experiences that captivate audiences across multiple formats.
      </p>
    </section>
  );
}

export default About;